...
- [ ] **luffy/verl/verl/workers/rollout/vllm_rollout/vllm_rollout.py:43** - 1. support pp in vllm 2. passing tokenizer is not necessary? no encoding/decoding is happending here 3. simplify init logics NOTE(sgm): add for verl. We can optimize it by making the dataloader yield List[int] without padding.
